...
<div class="max-w-6xl mx-auto">
    ...
    <?php
        ...
        try {
            ...
            if (!$hasOpenDispute && in_array($order['status'], ['delivered', 'revision_requested'])) {
                $currentUserId = Auth::user()['id'];
                $isPartyToOrder = ($order['client_id'] === $currentUserId || $order['student_id'] === $currentUserId);
                $canOpenDispute = $isPartyToOrder;
            }
        } catch (Exception $e) {
            // Silently fail
        }
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm text-yellow-700">
                        <span class="font-bold">Order is in Dispute</span>
                        <br>
                        This order is currently under investigation. Actions such as delivery, acceptance, and revision requests are paused until the dispute is resolved.
                    </p>
                </div>
            </div>
        </div>
